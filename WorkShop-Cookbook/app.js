...
function renderRecipes(recipes) {
    ...
    recipes.forEach(x => {
        mainElement.appendChild(createRepice(x));
    });

}

function createRepice(recipe) {
    ...
    recipeElement.addEventListener('click', () => {
        fetch(`${baseUrl}/jsonstore/cookbook/details/${recipe._id}`)
            ...
            .then(details => {
                ...
                mainElement.appendChild(renderDitailedRecipe(details));
            })
    })
...
function renderDitailedRecipe(details) {
    let recipeElement = document.createElement('article');
